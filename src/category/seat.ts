...
export async function bookSeat(
  ...
  const start: string | number =
    typeof startTime === "string" ? startTime : startTime * 60 || 8 * 60;
  const end: string | number =
    typeof endTime === "string" ? endTime : endTime * 60 || 22 * 60;
  ...
  try {
    const { data: originalData }: AxiosResponse<string> = await axios.get(
      `${URLP}/saveBook?${QUERY_STRING}`,
      {
        headers: {
          Cookie: cookie,
        },
...
/** 释放座位（已使用的情况下 */
export async function stopBookSeat(
  cookie: string,
  bookId: number
): Promise<BookSeatResult> {
  const { data: data }: AxiosResponse<string> = await axios.get(
    `${URLP}/stop?bookId=${bookId}`,
...
/** 取消座位（未使用的情况下 */
export async function cancelBookSeat(
  cookie: string,
  bookId: number
): Promise<BookSeatResult> {
  const { data: data }: AxiosResponse<string> = await axios.get(
    `${URLP}/cancel?bookId=${bookId}`,
    { headers: { Cookie: cookie } }
  );
  return JSON.parse(data);
}
...
/** 加载空闲时间 */
export async function loadBookSeatTime(
  cookie: string,
  seatId: number,
  ...
  try {
    const { data: data }: AxiosResponse<string> = await axios.get(
      `${URLP}/loadStartTime?${QUERY_STRING}`,
      { headers: { Cookie: cookie } }
    );
