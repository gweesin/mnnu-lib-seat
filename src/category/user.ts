 ...
 */
export async function getUserBookHistory(
  cookie: string
): Promise<UserBookHistory> {
  try {
    const response: AxiosResponse<Result> = await axios.get(
      `${URL}/libseat-ibeacon/getUserBookHistory`,
      {
        headers: {
          Cookie: cookie,
          Referer:
            "https://seatwx.mnnu.edu.cn/libseat-ibeacon/currentBook?linkSign=currentBook",
        },
      }
    );
