...
export class OccupySeat {
    ...
    constructor(user: User, expects: UserExpected[], occupyBegin?: number, occupyEnd?: number) {
        this.user = user;
     ...
     */
    public async occupyExpectedSeat(cookie: string, beginHour?: number, endHour?: number) {
                ...
                let interval: NodeJS.Timer = setInterval(() => {
                    ...
                    if (this.occupyBegin < now && now < this.occupyEnd) {
                        for (let i = 0; i < (expect.priority || 1); ++i) {
                            bookSeat(cookie, expect.seatId, date, beginHour, endHour).then(async (resp: BookSeatResult) => {
                                console.log(resp);
                                if (!resp || !resp.message || resp.message.search('登录失败: 用户名或密码不正确') !== -1 ||
